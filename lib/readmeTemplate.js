...
const licenseNotice = require("./licenseNotices"); //contains logic for notice selection

function generatesReadme(data) {

    const codeSnippetTemplate = (codeBlock) => `
\`\`\`
${(codeBlock)}
\`\`\`
    `
...
${(data.installationCodeSnippet ? codeSnippetTemplate(data.installationCodeSnippet) : "")}
`,
        usageSection:
            `
## Usage
...
${(data.usageCodeSnippet ? codeSnippetTemplate(data.usageCodeSnippet) : "")}
...
${(data.testsCodeSnippet ? codeSnippetTemplate(data.testsCodeSnippet) : "")}
`,
        questionsSection:
            `
## Questions
